...
package com.commit451.verticaldrawerarrowdrawable;
 ...
 * </p>
 */
public class VerticalDrawerArrowDrawable extends Drawable {

    /**
     * Direction to make the arrow point upward.
     *
    ...
    /**
     * Direction to make the arrow point downward.
     *
     * @see #setDirection(int)
     * @see #getDirection()
     */
    ...
    @IntDef({ARROW_DIRECTION_UP, ARROW_DIRECTION_DOWN})
    ...
    // The arrow direction
    @ArrowDirection
    private int mDirection = ARROW_DIRECTION_UP;

    /**
     * @param context used to get the configuration for the drawable from
     */
    public VerticalDrawerArrowDrawable(Context context) {
        ...
        setBarThickness(context.getResources().getDimension(R.dimen.vdad_thickness));
        setSpinEnabled(true);
        // round this because having this floating may cause bad measurements
        setGapSize(Math.round(context.getResources().getDimension(R.dimen.vdad_gapBetweenBars)));

        mSize = context.getResources().getDimensionPixelSize(R.dimen.vdad_drawableSize);
        // round this because having this floating may cause bad measurements
        mBarLength = Math.round(context.getResources().getDimension(R.dimen.vdad_barLength));
        // round this because having this floating may cause bad measurements
        mArrowHeadLength = Math.round(context.getResources().getDimension(R.dimen.vdad_arrowHeadLength));
        mArrowShaftLength = context.getResources().getDimension(R.dimen.vdad_arrowShaftLength);
    }
    ...
    public void draw(Canvas canvas) {
        ...
        final boolean flipToPointDown;
        switch (mDirection) {
            case ARROW_DIRECTION_UP:
                flipToPointDown = false;
                break;
            case ARROW_DIRECTION_DOWN:
                flipToPointDown = true;
                break;
            default:
                flipToPointDown = false;
        }
        ...
        final float canvasRotate = lerp(0, flipToPointDown ? -90 : 90, mProgress);
        ...
        if (mSpin) {
            ...
            canvas.rotate(flipToPointDown ? -90 : 90);
        }
    ...
    private int getThemeAttrColor(Context context, int attributeColor) {
